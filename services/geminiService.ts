...
export const suggestDriver = async (booking: Booking, drivers: Driver[]): Promise<AiDriverSuggestion | null> => {
  try {
    ...
    const prompt = `
      Act as an expert fleet dispatcher in London. Analyze this booking and the available drivers to suggest the best match.
      ...
      Consider vehicle capacity, proximity (assume locations are logical for London), and driver status.
...
export const generateWhatsAppMessage = async (booking: Booking, driver: Driver): Promise<string> => {
  try {
    const prompt = `
      You are EliteDispatch, a premium London chauffeur dispatch system. Write a WhatsApp message to driver ${driver.name} assigning them a job.
      
      Job Details:
      - Customer: ${booking.customerName}
      - Pickup: ${booking.pickupLocation}
      - Dropoff: ${booking.dropoffLocation}
      - Time: ${new Date(booking.pickupTime).toLocaleTimeString('en-GB', {hour: '2-digit', minute:'2-digit'})}
      - Pax: ${booking.passengers}
      - Price: £${booking.price}
      - Notes: ${booking.notes || 'N/A'}

      Format guidelines:
      - Use British English spelling (e.g. Centre, Colour).
      - Start with "🆕 *NEW JOB*"
    ...
    return `🆕 *NEW JOB*\n📍 Pick: ${booking.pickupLocation}\n🏁 Drop: ${booking.dropoffLocation}\n🕒 Time: ${new Date(booking.pickupTime).toLocaleTimeString('en-GB')}\n\nReply 👍 to confirm.`;
  }
};